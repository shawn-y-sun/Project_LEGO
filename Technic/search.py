            ...
            with warnings.catch_warnings():
                ...
                try:
                    ...
                    if parallel and total > 1:
                        ...
                        dm_locked = True
                        print("[parallel] DataManager locked for read-only threaded execution.")
        
                        # NOTE: The ThreadPoolExecutor shares the interpreter lock. Heavy
                        # work inside ``assess_spec`` (e.g., statsmodels OLS fitting or
                        # NumPy linear algebra) can execute on different CPU cores because
                        # those libraries release the GIL. Pure-Python blocks such as
                        ...
                        print(f"[parallel] Launching {worker_count} worker threads for {total} specs.")
                        if assignments:
                            print(f"[parallel] Thread spec allocation: {assignments}")
        
                        processed = 0
                        with ThreadPoolExecutor(max_workers=worker_count) as executor:
                            futures = [executor.submit(_evaluate, arg) for arg in worker_args]
    ...
    def run_search(
        ...
        **legacy_kwargs: Any
        ...
        parallel : bool, default False
            Enable threaded assessment of candidate specs. Set to True to
            evaluate combinations concurrently while assuming read-only access to
            the shared DataManager caches. Only work that drops into C/Fortran
            extensions (NumPy, pandas, statsmodels) can bypass the Python GIL, so
            workloads dominated by pure-Python feature preparation may still run
            effectively serially even when threading is enabled.
        max_workers : int, optional
            Maximum number of threads used when ``parallel`` is True. ``None``
            lets ``ThreadPoolExecutor`` decide; non-positive values are ignored.
